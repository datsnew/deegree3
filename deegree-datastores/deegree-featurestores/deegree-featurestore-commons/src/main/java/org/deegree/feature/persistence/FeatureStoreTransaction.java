 ...
 * <p>
 * Please note that a transaction must always be ended by calling either {@link #commit()}
 * or {@link #rollback()}.
 * </p>
 *
...
public interface FeatureStoreTransaction {
	 ...
	 * {@link FeatureStore}.
	 * @throws FeatureStoreException if the committing fails
	 */
	 ...
	 * {@link FeatureStore}.
	 * @throws FeatureStoreException if the rollback fails
	 */
	public void rollback() throws FeatureStoreException;

	/**
	 * Inserts the given {@link FeatureCollection} into the {@link FeatureStore}
	 * (including subfeatures).
	 * @param fc features to be inserted, must not be <code>null</code>
	 * @param mode mode for deriving the ids of the inserted objects, must not be
	 * <code>null</code>
	 * @return effective ids of the inserted feature and subfeatures (in document order)
	 * @throws FeatureStoreException if the insertion fails
	 */
	 ...
	 * @param ftName feature type of the features to be updated, must not be
	 * <code>null</code>
	 * @param replacementProps properties and their replacement values plus the action to
	 * take, must not be <code>null</code>
	 * @param filter selects the feature instances that are to be updated, must not be
	 * <code>null</code>
	 * @param lock optional lock object, may be <code>null</code>
	 * @return ids of updated feature instances, never <code>null</code>
	 * @throws FeatureStoreException if the update fails
	 */
	...
	/**
	 * Deletes the features from the {@link FeatureStore} that are matched by the given
	 * filter and type.
	 * @param ftName feature type of the features to be deleted, must not be
	 * <code>null</code>
	 * @param filter filter that determines the features to be deleted, must not be
	 * <code>null</code>
	 * @param lock optional lock object, may be <code>null</code>
	 * @return number of deleted feature instances
	 * @throws FeatureStoreException if the deletion fails
	 */
	 ...
	 * @param lock optional lock object, may be <code>null</code>
	 * @return number of deleted feature instances
	 * @throws FeatureStoreException if the deletion fails (*not* if a specified id does
	 * not exist)
	 */
